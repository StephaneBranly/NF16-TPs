...
T_Transaction* ajouterTransaction(int idEtu, float montant, char *descr, T_Transaction *listeTransaction){
    ...
    nouvelle_description_p=malloc(sizeof(descr));
    strcpy(nouvelle_description_p,descr);

    PremElt=malloc(sizeof(T_Transaction));
    PremElt->idEtu=idEtu;
...
void Tout_Liberer(BlockChain bc){
    ...
    while(Block_Prochain!=NULL){
        ...
        while(TransactionProchaine!=NULL){
            ...
            free(TransactionActuelle->descr);
...
/*char* Creer_Description(char* description){
    char *description_fixe;
    description_fixe=malloc(sizeof(description));
    description_fixe=description;
    return description_fixe;
}
*/
